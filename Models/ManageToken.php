...
class ManageToken {
    ...
    public function __construct($db, $expiryHrs = 5000000, $enc_char_len = 20, $date_format = "Y-m-d H:i:s") {
    ...
    public function createSessToken($obj_id, $dev_name, $mac_addr, $push_token, $user_type, $curr_time) {
...
//                $gmt_date = gmdate($this->dateFormat, $curr_time); //Converts date to GMT
        ...
        $local_exp_date = date($this->dateFormat, strtotime($curr_time) + $exp_seconds);

        $gmt_exp_date = gmdate($this->dateFormat, strtotime($curr_time) + $exp_seconds); //Converts date to GMT by adding expiry date in seconds

        $flag_after_insert = $this->insert_in_db($obj_id, $this->sess_token, $mac_addr, $this->user_device_type, $curr_time, $local_exp_date, $push_token, $user_type);
    ...
    public function updateSessToken($obj_id, $mac_addr, $push_token, $user_type, $curr_time) {
...
//                $curr_time = time();
        ...
        $flag_after_insert = $this->update_in_db($obj_id, $this->sess_token, $mac_addr, $local_exp_date, $push_token, $user_type);
    ...
    public function validateSessToken($obj_id, $token, $user_type) {

        $getTokenQry = "select sid from " . $this->db_session_table . " where oid='" . $obj_id . "' and token = '" . $token . "' and user_type = '" . $user_type . "'";
    ...
    public function revokeSessToken($obj_id, $user_type) {

        $getTokenQry = "delete from " . $this->db_session_table . " where oid='" . $obj_id . "' and user_type = '" . $user_type . "'";
    ...
    private function update_in_db($obj_id, $token, $mac_addr, $gmt_exp_date, $push_token, $user_type) {

        if ($push_token != '0')
            $push_string = ",push_token = '" . $push_token . "'";

        $updateQry = "update user_sessions set token = '" . $token . "',expiry = '" . $gmt_exp_date . "',loggedIn = '1' " . $push_string . " where oid = " . $obj_id . " and device = '" . $mac_addr . "' and user_type = '" . $user_type . "'";

        if ($user_type == 1 && $push_token != '0') {
            $location = $this->mongo->selectCollection('location');

            $setArr = array('pushToken' => $push_token);
            $newdata1 = array('$set' => $setArr);

            $location->update(array("user" => (int) $obj_id), $newdata1);
        }
    ...
    private function insert_in_db($obj_id, $token, $mac_addr, $type, $curr_date, $exp_date, $push_token, $user_type) {

        $insertQry = "insert into " . $this->db_session_table . "(oid,token,expiry,device,type,create_date,push_token,user_type) values(
            " . $obj_id . ",
                '" . $token . "',
                    '" . $exp_date . "',
                        '" . $mac_addr . "',
        ...
        if ($user_type == 1) {
            ...
            $setArr = array('pushToken' => $push_token, 'User_type' => $type);
            $newdata1 = array('$set' => $setArr);
