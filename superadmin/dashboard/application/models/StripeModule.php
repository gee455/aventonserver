...
class StripeModule {
    ...
    public function __construct() {
        ...
        Stripe::setApiKey(Stripe_Id); //Pranay account: publish-key  pk_test_3Qg1fAkeuC5WG9JBtkHBQZ3Q  
...
//        $card->name = $args["name"];
    ...
    private function createRecipient($args) {
        ...
        return Stripe_Recipient::create(array(
                    ...
                    "tax_id" => $args["tax_id"],
                    "bank_account" => array(
                        "country" => "US", //$args["country"],
                        "routing_number" => $args["routing_number"],
                        "account_number" => $args["bank_account"]
                    ),
                    "email" => $args["email"],
                    "description" => $args["description"]
    ...
    public function apiStripe($method, $args) {
        ...
        try {
            ...
            return array("error" => array("message" => "Invalid parameters were supplied to Stripes API"), 'err' => $e->getJsonBody());
